function [v,psnrall] = gapdenoise1( y , opt )
addpath('NGmeet')
addpath('assess_fold')
...
maxiter  = 1;     % maximum number of iteration
...
% Par   = ParSetH(5,31);
% [v,~,~]= NGmeet_DeNoising1( y, opt);  %NGmeet denoisng function 
% v = single(v/255);
% psnrall = [];
...
%                 v = wnnm_vdenoise(v,[],opt); % opt.sigma
                Par   = ParSetH(5,31);
                [v]= NGmeet_DeNoising( 255*double(v), 255*double(opt.orig),Par,iter);  %NGmeet denoisng function
%                [v]= NGmeet_DeNoising1( y,opt);  %NGmeet denoisng function 
                v = single(v/255);
            otherwise
