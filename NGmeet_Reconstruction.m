function [E_Img PSNR SSIM]   =  NGmeet_Reconstruction( par,rate, A, At  )
...
[h, w]           =    size(rec_im);  % size of the 2D image

Par   = ParSetH_reconstruction(5,w,rate);
N_Img = reshape(rec_im,[sqrt(h) sqrt(h) w]);
delta =1;
...
% estimation, can be ignored to speed up
    PSNR     =   csnr( reshape(E_Img,h,w), par.ori_im, 0, 0 );
...
if isfield(par,'ori_im')
    PSNR     =   csnr( reshape(E_Img,h,w), par.ori_im, 0, 0 );
    SSIM      =  cal_ssim( reshape(E_Img,h,w), par.ori_im, 0, 0 );
end
return;
